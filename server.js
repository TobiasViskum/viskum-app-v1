...
app.get('/api/fitness/get/remaining_exercises', (req, res) => {
    ...
    db.query(`SELECT DISTINCT A.name FROM fitness_stat_table AS A WHERE A.name NOT IN (SELECT DISTINCT B.name FROM fitness_stat_table AS B WHERE B.profile = '${profile}' AND B.${toggle} = 1) ORDER BY A.name`,
    (err, result) => {
        if (err) {
            console.log(err);
        } else {
...
app.get("/api/fitness/get", (req, res) => {
    const profile = req.query.profile
    const toggle = req.query.toggle == 1 ? 'has_reps' : 'has_max'
    const sortMode = req.query.toggle == 1 ? 'D' : 'B'

    db.query(`SELECT A.*, F.show_vas_fitness, F.show_vas_cardio, C.unit_name, B.group_name AS group_name_max, D.group_name AS group_name_reps, E.reps_range_name FROM fitness_stat_table A LEFT JOIN dim_grouping B ON A.group_id_max = B.group_id LEFT JOIN dim_grouping D ON A.group_id_reps = D.group_id INNER JOIN dim_units C ON A.unit_id = C.unit_id INNER JOIN dim_reps_range AS E ON A.reps_range_id = E.reps_range_id INNER JOIN dim_profile AS F ON A.profile = F.uid WHERE A.profile=(?) AND ${toggle}=1 ORDER BY ${sortMode}.group_sort_order ASC, ${sortMode}.group_name ASC, A.name ASC`, [profile],
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
...
app.put("/api/fitness/update/specific", (req, res) => {
    ...
    const newAmount = valueName == 'date' ? (req.body.newAmount ? (new Date(req.body.newAmount)) : null) : req.body.newAmount
   ...
   if (foundValueName == 'name') {
    db.query('SELECT id FROM fitness_stat_table WHERE name=? AND profile=?', [newAmount, profile],
    (err, result) => {
        if (err) {
            console.log(err);
        } else {
            if (result.length == 0) {
                db.query('UPDATE fitness_stat_table SET name=? WHERE id=?', [newAmount, id],
                (err, result) => {
                    if (err) {
                        console.log(err);
                    } else {
                        db.query('UPDATE fitness_stat_history SET exercise=? WHERE exercise=? AND profile_uid=?', [newAmount, prevName, profile],
                        (err, result) => {
...
app.put("/api/fitness/add", (req, res) => {
    ...
    db.query(`INSERT INTO fitness_stat_table SET profile=(?), name=(?), has_max=(?), has_reps=(?), unit_id=1 ON DUPLICATE KEY UPDATE has_max=(?), has_reps=(?);`,
        [profile, newName, addMax, addReps, addMax, addReps],
        (err, result) => {
            if (err) {
                console.log(err);
            } else {
...
app.get('/api/chat/get', (req, res) => {
    db.query('SELECT A.*, B.name FROM chat_stat_table AS A INNER JOIN dim_profile AS B ON A.profile = B.uid', 
    (err, result) => {
        if (err) {
            console.log(err);
        } else {
